...
| Local pressure model | **shipped** — `Shared/Pressure/LocalPressureModel.swift` + `HourlyPressureGrid.swift`. AR(*k*) + intercept, plus solar S1/S2 harmonics at the top size, closed-form least squares over a 30-day hourly grid, ridge `1e-6` so a flat log degrades instead of going singular. **The size is chosen from the log, not fixed**: `specificationLadder` tries AR(3)+S1+S2 (8 params) → AR(3) (4) → AR(2) (3) → AR(1) (2) and takes the first the data supports, gated on `max(6 rows, 1.5 x parameters)`. Fewer lags cost a parameter *and* buy rows — a design row for AR(*k*) needs *k* consecutive hours behind it — which is what makes the ladder work rather than merely lower the bar. Behind `LocalPressureModel.isOrderLadderEnabled`. Range **3–6 h** (`ForecastSource.localModel.rangeSeconds`) — a human decision, not a default: one sensor at one point cannot see advection, so extending it needs a measurement first. Fits from **one waking day** of history. The previous claim here — one day, >=12 rows — assumed 24 hourly cells a day, which a phone does not deliver: iOS grants `BGAppRefreshTask` sparsely and nothing overnight, so a real day is a run of waking hours beside a 12-plus-hour hole, and a hole restarts the count of consecutive lags. Measured on a real device log (29 readings / 23 h / 10 observed cells) the fixed 8-parameter fit had 6 of the 12 rows it wanted and returned `nil`; the ladder lands on AR(3) with those same 6 rows. The absolute floor of 6 rows is the range in hours: a model may not project further ahead than the number of hour-to-hour transitions it has actually observed. The forward iteration is seeded from the latest run of **three consecutive** hourly cells and its range is measured from that anchor, not from the clock: after an overnight gap the model says less, or nothing, rather than treating a twelve-hour jump as a one-hour step. Altitude excursions are rejected before the fit by the §3 rate gate; gaps ≤2 h are bridged and never counted as coverage. Coverage is measured over the **span the log reaches across**, not over the nominal 30-day window: on a one-day-old install 29 of those days are hours the app did not exist for, and counting them as holes inflated a complete cold-start log's band fivefold — ±22 hPa at six hours, enough to set the whole y-domain of the chart. The band is scaled from the **residual degrees of freedom** (`rows - parameters`), not the row count: dividing by `n` understates the spread by `sqrt(n/(n-p))`, 10% on a full fit and 73% on a six-row one, i.e. narrowest exactly where it should be widest. A six-row AR(3) fit on the real device log reports **+/-10.6 hPa at 6 h** against a full fit's +/-4.4 — the model saying it knows less. Refit at most daily, on a foreground activation — **no new wake source** |
