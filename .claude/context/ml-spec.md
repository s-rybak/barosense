...
- **Plausibility gate.** `Pressure.isPlausible` (finite, and 800–1100 hPa) already rejects
  unit mix-ups and bad reads at the boundary. `isFinite` is asserted explicitly rather than
  left to IEEE comparison semantics: the range check alone answers `false` for NaN only
  because every comparison against NaN is false, which stops being true the moment the line
  is rewritten as the equivalent-looking `!(x < 800 || x > 1100)`. One NaN reaching the
  hourly grid turns every mean, SD and delta over that window into NaN, in silence.
  `PressureTests` pins NaN, both infinities, and the two range bounds.
